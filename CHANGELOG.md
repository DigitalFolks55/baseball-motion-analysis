...
- Dual-mode FastAPI browser UI for video upload, media library browsing, and HTML5 replay.
- SQLite media metadata repository and local filesystem file store for browser-uploaded videos.
- Media-ID-based video content endpoint with HTTP byte-range support for browser seeking.
- Media-ID-based uploaded-video deletion from the browser library, including stored file and metadata cleanup.
- Documentation for local browser mode, server mode configuration, upload limits, browser codec limitations, and privacy limits.
...
- Reintroduced `python-multipart` for DEV002-01 browser upload handling, which explicitly authorizes a browser upload adapter.
